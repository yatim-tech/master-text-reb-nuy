...
3. **Penyelamatan VRAM (*Memory Optimization*)**:
   ...
   - Jika `OOM` pada **DPO**: Langsung belah dua `batch_size` — tidak ada tuas `--packing` setara untuk jalur ini. DPO memuat *reference model* berdampingan *policy model* → footprint VRAM ~2× SFT pada ukuran yang sama; jangan matikan `use_lora` default (aktif dari bucket `2_4_b` ke atas) demi hemat VRAM karena akan mengubah karakter pelatihan.
